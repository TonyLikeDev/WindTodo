...
export default function TopBar({ onMenuToggle = () => {} }: { onMenuToggle?: () => void }) {
  ...
  return (
    <header className="h-20 flex items-center justify-between px-6 md:px-10 border-b border-white/5 bg-black/10 backdrop-blur-lg">
        <div className="flex items-center space-x-4 flex-1 min-w-0 mr-4">
            ...
            <div className="relative hidden sm:block">
              ...
              <input 
                type="text" 
                placeholder="Search..." 
                className="w-48 lg:w-64 bg-white/5 border border-white/10 rounded-full py-1.5 pl-9 pr-4 text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:ring-1 focus:ring-white/20 focus:w-64 lg:focus:w-80 transition-all duration-300" 
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
