...
export default function Sidebar() {
  ...
  // Count all pending (non-done) tasks across all lists
  const pendingCount = Object.values(lists).flat().filter(t => t.status !== 'done').length;
  ...
  const navItems = [
    ...
    },
    {
      ...
      href: "/statistics",
      badge: null,
      icon: <><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.488 9H15V3.512A9.025 9.025 0 0120.488 9z"></path></>,
    },
    {
      name: "Users",
      href: "/users",
      badge: null,
      icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 01-12 0v1zm0-11a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0z"></path>,
    },
  ];

  return (
    <aside className="sidebar-glass w-64 flex-shrink-0 flex-col hidden md:flex">
      <div className="p-6 border-b border-white/5">
        ...
        <div className="flex items-center space-x-3 mb-10">
          ...
          <div>
            ...
            <div className="text-[10px] text-accent-400/70 font-semibold uppercase tracking-widest">Pro</div>
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all group ${
                  isActive
                    ? "bg-accent-500/15 text-white border border-accent-500/30 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]"
                    : "text-gray-400 hover:bg-white/5 hover:text-white border border-transparent"
                }`}
              >
                <div className="flex items-center gap-3">
                  <svg className={`w-5 h-5 transition-colors ${isActive ? 'text-accent-400' : 'text-gray-500 group-hover:text-gray-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    {item.icon}
                  </svg>
                ...
                {item.badge !== null && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[1.25rem] text-center ${
                    isActive ? 'bg-accent-500/30 text-accent-300' : 'bg-white/10 text-gray-400 group-hover:bg-white/15'
                  }`}>
        ...
        {pendingCount > 0 && (
          <div className="mt-6 px-3 py-3 bg-accent-500/5 border border-accent-500/15 rounded-xl">
            <div className="text-[10px] text-accent-400/70 uppercase font-semibold tracking-wider mb-1">Pending</div>
            <div className="text-sm font-bold text-white">{pendingCount} task{pendingCount !== 1 ? 's' : ''} left</div>
          </div>
        )}
      </div>
